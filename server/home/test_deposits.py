...
def get_customers():
    token = get_token()
    url = 'https://mena-evest.pandats-api.io/api/v3/customers/deposits?page=4&limit=200&fields=email,createdTime,ftd,amount,email'
...
for d in datas:
    ...
    compared_date = "2021-06-24"
    comparedDateObj = datetime.strptime(compared_date, '%Y-%m-%d')
    if convertedDate >= startDateObj and convertedDate <= comparedDateObj and ftd == True:
        print("email: ", email)
        print("amount: ", amount)
        print("FTD: ", ftd)
        print("Time: ", createdTime)
        print("============")
        final_list.append(d)
